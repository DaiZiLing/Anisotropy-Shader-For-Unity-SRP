    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            real4 FRAG(v2f i): SV_TARGET
            {
                ...
                float4 Mask = SAMPLE_TEXTURE2D(_MaskMap, sampler_MaskMap, i.texcoord.xy); // MASK贴图，R为金属度、G为AO、B为detail normal mask、A为光滑度
                ...
                //这里按照迪士尼原则本来得用power 2的，但是由于粗糙度不用整成滑块了，所以直接用它本身
                ...
                N = normalize(lerp(N, AnisotropicN, AnisotropicStretch)); //这种做法叫bent reflection vector，这一块的法线把各向异性产生的粗糙度扰动考虑进去了
                //===============================================================================
                ...
                //直接漫反射项
                float3 ks = F; //F已经帮我们把Ks实现了
                float3 kd = (1 - ks) * (1 - Metallic);//kd漫反射的那部分，所以它的金属度是1 - Metallic
